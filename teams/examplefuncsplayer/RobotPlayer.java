...
public class RobotPlayer {
	// The first example player, most choices made are random.
	static RobotController rc; // all of these static instantiations will be used for every player
	static Team myTeam; 
	...
	public static void run(RobotController tomatojuice) {
		...
		while(true) {
            ...
            // If we are considering headquarters ...
			if (rc.getType() == RobotType.HQ) {
				try {					
					int fate = rand.nextInt(10000); // choose some random integer 
					myRobots = rc.senseNearbyRobots(999999, myTeam);
					// initially everything is set to zero, until we loop through and count up the number of each type of robot
					int numSoldiers = 0;
					int numBashers = 0;
					int numBeavers = 0;
					int numBarracks = 0;
					...
					// not exactly sure what this does here...
					...
					// be sure to look at delay table 
					if (rc.isWeaponReady()) {
						attackSomething();
					}
			...
			// we're going to want to change up this if statement
			// to consider only attacking if there is an adversary nearby, 
			// rather than shooting continously.. 
			...
			if (rc.getType() == RobotType.BASHER) {
                try {
					...
					if (rc.isCoreReady()) {
						int fate = rand.nextInt(1000); // randomly chosen fate
						if (fate < 800) {
			...
			// basically using the same strategy as we do for the bashers 
	...
	// converts a direction into an integer, the 8 cardinal directions 
	// incerementing integers by moving clockwise
